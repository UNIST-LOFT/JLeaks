public synchronized void init(boolean force) 
{
    ...
    try {
        ...
        if (dbFile.exists() || (se.getErrorCode() == 90048)) {
            // Cache is corrupt or a wrong version, so delete it
            ...
            try {
                ...
                if (!net.pms.PMS.isHeadless()) {
                    try {
                        JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor((Component) PMS.get().getFrame()), String.format(Messages.getString("DLNAMediaDatabase.ConnectionError"), dbDir), Messages.getString("Dialog.Error"), JOptionPane.ERROR_MESSAGE);
    ...
    try (Connection conn2 = getConnection()) {
        ...
        if (se.getErrorCode() != 42102) {
            // Don't log exception "Table "FILES" not found" which will be corrected in following step
                ...
                switch(version) {
                    case 23:
                        try (Statement statement = conn3.createStatement()) {
                            ...
                            sb.append("UPDATE ").append("FILES ").append("SET ").append("IMDBID = NULL, ").append("YEAR = NULL, ").append("MOVIEORSHOWNAME = NULL, ").append("MOVIEORSHOWNAMESIMPLE = NULL, ").append("TVSEASON = NULL, ").append("TVEPISODENUMBER = NULL, ").append("TVEPISODENAME = NULL, ").append("ISTVEPISODE = NULL, ").append("EXTRAINFORMATION = NULL ").append("WHERE ").append("NOT ISTVEPISODE");
                            statement.execute(sb.toString());
