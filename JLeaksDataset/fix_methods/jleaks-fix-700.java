private void optionContacts() 
{
    ...
    Cursor cursor = getContentResolver().query(ContactsContract.Contacts.CONTENT_URI, null, null, null, Phone.DISPLAY_NAME);
    if (cursor != null)
        try {
    ...
    for (Integer id : mapContact.keySet()) {
        ...
        ids[i] = id;
        selection[i++] = PrivacyManager.getSettingBool(null, this, String.format("Contact.%d.%d", mAppInfo.getUid(), id), false, false);
    }
    // Build dialog
    AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(this);
    alertDialogBuilder.setTitle(getString(R.string.menu_contacts));
    alertDialogBuilder.setIcon(getThemed(R.attr.icon_launcher));
    alertDialogBuilder.setMultiChoiceItems(listContact.toArray(new CharSequence[0]), selection, new DialogInterface.OnMultiChoiceClickListener() {

        public void onClick(DialogInterface dialog, int whichButton, boolean isChecked) {
            PrivacyManager.setSetting(null, ActivityApp.this, String.format("Contact.%d.%d", mAppInfo.getUid(), ids[whichButton]), Boolean.toString(isChecked));
        }
    });
