public ClientRequestExecutor create(SocketDestination dest) throws Exception 
{
    ...
    while (!socketChannel.finishConnect()) {
        ...
        if (diff < 0) {
            ...
            throw new ConnectException("Cannot connect socket " + numCreated + " for " + dest.getHost() + ":" + dest.getPort() + " after " + connectTimeoutMs + " ms");
        }
        if (logger.isTraceEnabled())
            logger.trace("Still creating socket " + numCreated + " for " + dest.getHost() + ":" + dest.getPort() + ", " + diff + " ms. remaining to connect");
