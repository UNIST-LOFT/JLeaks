public void generateReport(String baseReportDir, ReportProgressPanel progressPanel) 
{
    if (configPanel == null) {
        // NON-NLS
        logger.log(Level.SEVERE, "CASE/UCO settings panel has not been initialized");
    ...
    if (selectedDataSourceId == ReportCaseUcoConfigPanel.NO_DATA_SOURCE_SELECTED) {
        // NON-NLS
        logger.log(Level.SEVERE, "No data source selected for CASE/UCO report");
    ...
    try {
        ...
        // NON-NLS
        logger.log(Level.SEVERE, "Unable to create directory for CASE/UCO report", ex);
    ...
    try {
        ...
        // NON-NLS
        logger.log(Level.SEVERE, "Failed to get list of files from case database", ex);
        progressPanel.complete(ReportStatus.ERROR);
    } catch (IOException ex) {
        // NON-NLS
        logger.log(Level.SEVERE, "Failed to create JSON output for the CASE/UCO report", ex);
        progressPanel.complete(ReportStatus.ERROR);
    } catch (SQLException ex) {
        // NON-NLS
        logger.log(Level.WARNING, "Unable to read result set", ex);
        progressPanel.complete(ReportStatus.ERROR);
        ...
        if (jsonGenerator != null) {
            try {
                ...
                // NON-NLS
                logger.log(Level.WARNING, "Failed to close JSON output file", ex);
            }
        }
    }
}