public void setProject (File x) 
{
    ...
    if (!generateTemplate && output.exists() && questionsFile.lastModified() <= output.lastModified() && this.getProject().getProperty("arch.generate") == null) {
    ...
    }
    {
        ...
        if (!s.isEmpty()) {
            if ("true".equals(this.getProject().getProperty("arch.generate"))) {
                ...
                log(questionsFile.getAbsolutePath() + ": some questions have not been answered: " + s + "\n" + "Run with -Darch.generate=true to add missing questions into the end of question file", Project.MSG_WARN);
            }
        }
    }
    if (apichanges != null) {
    ...
    try {
        ...
        if (file != null) {
            ...
            ss = new StreamSource(getClass().getResourceAsStream("Arch.xsl"));
        }
